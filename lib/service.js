...
const { normalizeLevel, starLevelOfTitle } = require('./program');
...
/**
 * GET /advancement/index -> [{ trailmanId, name }] from #trailmen-select.
 * The two group options ("All Navigators"/"All Adventurers") are skipped —
 * their values are level ids, not trailman hashids.
 */
function parseTrailmenIndex(html) {
  ...
  if (!sel) return { trailmen: [], warnings: ['trailmen-select not found on /advancement/index'] };
  const out = [];
  const re = /<option\b([^>]*)>([\s\S]*?)<\/option\s*>/gi;
  let m;
  while ((m = re.exec(sel[1]))) {
    const value = attrOf(m[1], 'value');
  ...
  return { trailmen: out, warnings: out.length ? [] : ['trailmen-select held no trailman options'] };
}
