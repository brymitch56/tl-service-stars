...
/**
 * A slot's suffix is an advancement record id: "ad" followed by exactly ten
 * characters. The id pattern is part of the match on purpose — the form also
 * carries PAGE-level fields with the same prefixes, notably `comment-specified`
 * and `date-specified` (the "apply to all" controls). A looser pattern turns
 * `comment-specified` into a phantom award slot, and the echo then invents a
 * `new-specified` / `completed_on-specified` set of fields that never existed.
 */
...
function parseStandardFragment(html) {
  ...
  const slotOf = (adId) => {
    if (!byId.has(adId)) {
      // `hasNewField` tracks whether the portal sent a `new-` input at all.
      // An instance already on the record has NO such input (verified live,
      // 2026-09-19) — it is not sent as "false". Emitting one on the way back
      // would be inventing a field, so the echo only repeats what was there.
      byId.set(adId, {
  ...
  for (const i of inputsIn(body)) {
    ...
    switch (m[1].toLowerCase()) {
      case 'new': s.hasNewField = true; s.isNew = /^(true|1)$/i.test(i.value); break;
...
function toFields(parsed, fill, extra = {}) {
  ...
  for (const s of parsed.slots) {
    ...
    // Only repeat `new-` when the portal sent one. An existing instance has
    // none, and adding it would be a field the form never had.
    if (s.hasNewField) out[`new-${s.adId}`] = s.isNew ? 'true' : 'false';
