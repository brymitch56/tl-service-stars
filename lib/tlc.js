...
/**
 * A description of the forms on a page, for the log when sign-in lands
 * somewhere we did not expect. Field NAMES and types only — never a value, so
 * a password, a token or a texted code can never reach the journal.
 */
function describeForms(html) {
  return formsIn(html).map((f) => {
    const ins = inputsIn(f.body);
    const textareas = [...(f.body || '').matchAll(/<textarea\b([^>]*)>/gi)]
      .map((m) => attrOf(m[1], 'name')).filter(Boolean);
    const selects = [...(f.body || '').matchAll(/<select\b([^>]*)>/gi)]
      .map((m) => attrOf(m[1], 'name')).filter(Boolean);
    return {
      action: attrOf(f.attrs, 'action') || '(same page)',
      method: (attrOf(f.attrs, 'method') || 'GET').toUpperCase(),
      inputs: ins.map((i) => `${i.name}:${i.type}`),
      textareas,
      selects,
    };
  });
...
function makeClient(cfg, { store = NO_STORE, fetchImpl = fetch, sleep = null, log = () => {} } = {}) {
  ...
  /**
   * @param {{unsafe?:boolean, challenge?:boolean, signin?:boolean}} mode
   *   `signin` marks sign-in navigation. Signing in is not a data path and
   *   the portal decides where it goes: a correct password redirects to
   *   whatever verification or landing page it likes, and those paths are not
   *   knowable in advance. Holding them to the READ allow-list refuses the
   *   redirect and turns "please type the code we just texted you" into an
   *   opaque failure. So sign-in hops get the same narrower guard the code
   *   form gets — never a data-changing endpoint, never the save — instead of
   *   the read list.
   */
  async function send(pathOrUrl, opts = {}, { unsafe = false, challenge = false, signin = false } = {}) {
    ...
    const permissive = challenge || signin;
    if (permissive) guardChallenge(pathname);
    ...
    while (res.status >= 300 && res.status < 400 && res.headers.get('location') && hops++ < 5) {
      ...
      // Re-check the ORIGIN on every hop, not just the first. A Location
      // header is the portal telling us where to go next, and a redirect to
      // another host would hand that host our session cookies.
      if (new URL(next).origin !== new URL(cfg.base).origin) {
        fail(EXIT.CONFIG, `refusing to send portal cookies to ${new URL(next).origin}`);
      }
      if (permissive) guardChallenge(new URL(next).pathname);
      else if (!unsafe) guard(new URL(next).pathname, 'GET');
      const h = baseHeaders();
  ...
  async function probeSession() {
    ...
    let res;
    try { res = await send(at, {}, { signin: true }); } catch { return null; }
  ...
  async function login(opts = {}) {
    ...
    const loginPage = await send(cfg.loginPath, {}, { signin: true });
    ...
    // No code form recognised and not the login form either. Before giving up,
    // record what the page actually held — field NAMES and types only, never a
    // value — so an unfamiliar second-factor form can be identified from the
    // log instead of by sending the human another code to look at.
    if (!/logout|sign\s*out|dashboard/i.test(after)) {
      log(`[tlc] sign-in landed on an unrecognised page; forms: ${JSON.stringify(describeForms(after))}`);
      fail(EXIT.AUTH, 'The portal answered the password with a page this app does not recognise. '
        + 'The server log lists the form fields it saw.');
    }
  ...
  async function fetchBadgeTrackerView({ trailmanId, awardId, level }) {
    // These parameter names are the AJAX view's own, and are deliberately
    // NOT the form's field names (captured from the live page, 2026-09-19):
    // send the form's names here and the portal answers "This action can
    // only be used in AJAX mode." with a 42-byte body.
    const body = new URLSearchParams();
    body.set('_csrf', csrf || '');
    body.set('style', 'standard');
    if (level) body.set('level', level);
    body.append('trailmen[]', trailmanId);
    body.set('badges', awardId);
    body.set('lockedChecked', 'false');
    body.set('event_id', '');
    body.set('track_attendance', 'false');
  ...
  async function postAdvancementIndex(fields) {
    // Ordered [name, value] pairs — the form repeats names, and order is part
    // of echoing it faithfully. An object is accepted too, for convenience.
    const body = new URLSearchParams();
    const pairs = Array.isArray(fields) ? fields : Object.entries(fields);
    for (const [k, v] of pairs) {
      if (Array.isArray(v)) for (const one of v) body.append(k, String(one));
    ...
    // A session that expired mid-save bounces to the sign-in form. That is an
    // auth failure, not a save — say so plainly and stop, so the caller holds
    // the row instead of reading back against a signed-out page.
    if (/LoginForm\[password\]/.test(html)) {
      fail(EXIT.AUTH, 'the portal session was lost during the save — stopping, and not retrying');
    }
    return html;
  }
...
module.exports = {
  ...
  parseChallenge, challengePrompt, describeForms, guard, guardChallenge,
